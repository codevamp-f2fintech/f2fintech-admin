...
const Ticket = () => {
  ...
  const { value: ticketData, error: swrError, swrLoading, refetcher } = useGetTickets(
    apiEndpoint,
    currentPage,
    ITEMS_PER_PAGE,
    filter,
  ...
  useEffect( () => {
    ...
    const fetchUsers = async () => {
      try
      {
        ...
        setUserData( ( userRole === "admin" || userRole === "sub admin" ) ? sortedUsers : sortedUsers?.filter( user => user.role === userRole ) );
      } catch ( error )
      {
  ...
  useEffect( () => {
    ...
    {
      const foundUser = userData?.find( user => user.id === parseInt( queryUserId ) );
