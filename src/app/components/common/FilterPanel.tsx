            ...
            <Box sx={{ display: 'flex', gap: 1, alignItems: 'center', flexWrap: 'wrap' }}>
                ...
                <Tooltip title="Filter by Status">
                    <Chip
                        ...
                        label={`${sortBy.charAt(0).toUpperCase() + sortBy.slice(1)} (${ticketCount(sortBy)})`}
                ...
                >
                    {['all', 'to do', 'in progress', 'forwarded', 'done'].map((status) => (
                        <MenuItem
                            key={status}
                            onClick={() => {
                                handleSortChange({ target: { value: status } });
                                setAnchorEl(null);
                            }}
                            sx={{
                                gap: 1,
                        ...
                        >
                            ...
                            {`${status.charAt(0).toUpperCase() + status.slice(1)} (${ticketCount(status)})`}
                        </MenuItem>
                    ))}
                </Menu>
