...
export const useGetLoanProviders = (
    ...
    const { data: swrData, error, isValidating } = useSWR<LoanProvider | null>(
...
export const useCreateLoanProvider = (pathKey: string) => {
    ...
    const createLoanProvider = async (newLoanProviderData: LoanProviderData) => {
        ...
        setError(null);
        try {
            const resp = await creator<LoanProviderData, LoanProviderData>(pathKey, newLoanProviderData);
            return resp;
        } catch (err) {
...
export const useModifyLoanProvider = (pathKey: string) => {
    ...
    const modifyLoanProvider = async (updatedLoanProviderData: Partial<LoanProviderData>) => {
        setLoading(true);
        setError(null);
        try {
            const loanProvider = await modifier<LoanProviderData, Partial<LoanProviderData>>(pathKey, updatedLoanProviderData);
            return loanProvider;
        } catch (err) {
            setError(err as Error);
        } finally {
